...
/** Agrupa por día, igual que la agenda: leer una lista plana de fechas cuesta. */
...
export default async function SolicitudesPage({
  ...
  // La cita que se acaba de aceptar viaja por la URL: la lista ya no la
  // contiene (dejó de estar `requested`) y la liga se necesita justo ahora.
  const tokenAceptada =
    typeof params.aceptada === "string" ? params.aceptada : null;
  const { data: aceptada } = tokenAceptada
    ? await supabase
        .from("appointments")
        .select(
          "access_token, starts_at, ends_at, patients(name, phone, is_minor, tutor_name, tutor_phone)",
        )
  ...
  const plantilla = base.includes("{liga}")
    ? base
    : `${base} Aquí puedes confirmar: {liga}`;
  ...
  return (
    <>
      ...
      {vencidas.length > 0 && (
        <section className="mb-8 rounded-marca border border-border bg-surface-2 px-4 py-4 sm:px-5">
          ...
          <form action={marcarVencidas} className="mt-4">
            <button className="boton boton-suave text-xs">
              Archivar vencidas
            </button>
          </form>
        </section>
      ...
      {pendientes.length === 0 ? (
        <EstadoVacio
          titulo={
            hayFiltros(filtros) ? "Nada coincide" : "Sin solicitudes por ahora"
          }
        >
          {hayFiltros(filtros)
                    ...
                    <li key={s.id} className="tarjeta p-4 sm:p-5">
                      ...
                      {compiten && (
                        <p className="mt-3 text-xs font-medium text-alerta">
                          Otro paciente pidió este mismo horario. Al aceptar
                          uno, el otro se queda sin lugar.
                        </p>
                      )}
